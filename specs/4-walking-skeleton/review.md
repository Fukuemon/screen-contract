...
## Review 2026-08-23 (track gate 3 回目)

Verdict: **NEEDS_WORK**

前回の 4 件は 2 件が解消、1 件が一部、1 件が未解消。

### 指摘 (track gate 3 回目)

1. **テスト観点が D26 と矛盾する。** 不応答の観点が「core/execution が**再作成の判断**を下せること」のままで、D26 (再作成せず `run-failed` で終える) と食い違う。加えて「adapter が黙って再作成しないこと」(D4 の眼目) が観点から抜けており、そのままだと「黙って再作成されると `skipped` を観測できなくなる」の担保が無い。
2. **D28 を採ると `ir-version-changed` の除外 (D24) が成立しない。** ADR-0018 と execution feature は差し替えの契機を「**pause 中に draft が変わった状態で** `resume` またはステップ指定の再実行を要求されたとき」と定める。トリガは draft の変更であって承認ではない。記録は draft を書く操作であり、記録用 run は `paused` の間に draft が変わったうえで `resume` される。IR 版は「正規化した IR の内容から決まる値」であるため版が変わり、`resume` の直前に差し替えが起きて発行される。除外理由「その draft を**確定させずに** `resume` して終え」は、確定 (承認) と draft 変更を取り違えている。
3. **整合表の web-editor が「継承」のまま。** 影響表には web-editor の行が 2 本 (フレーム識別子の制約 / `input-discarded` の語彙) ある。
4. (軽微) **Sequence 2 の不応答分岐の後、処理が評価へ続いて見える。** フェーズ表の備考も D26〜D28 がどの phase の産物か読めない。

### 確認された整合 (問題なし)

- **D26 は D4 を骨抜きにしていない。** D4 の内容は (1) Port が構造化エラーを返す、(2) adapter が黙って再作成しない、(3) 再作成の可否は core/execution が決める、の 3 点で、D26 は (3) の選択肢のうち skeleton が採る側を固定するだけである。D4 自体の書き換えは不要。
- `rolled-back` と `run-aborted` の除外理由は正確である。前者は `rerun_step` が利用者指定の再実行であって前提不一致の巻き戻しとは別であること、後者は D28 が記録用 run を `resume` で終えることによる。
- **`session-recreated` を「語彙に足すが skeleton では発行しない」とする扱いは妥当。** D4 の要求が durable な設計であること、発行しないことが 3 箇所で明示され実装漏れと区別できること、D8 / D14 が同型の扱いを既に採っていることによる。
- 5 表とメタは指摘 3 の 1 行を除いて一致している。
...
## Review 2026-08-23 (最終 gate) — sync + prompts を累積

Verdict: **NEEDS_WORK**

観点別では上位文書整合 / 未解決論点 / 実装対象明示 / EARS acceptance / 正本境界が PASS。NEEDS_WORK は template 必須節・外部依存の健全性・prompts 自己完結性の 3 つ。

### 指摘 (最終 gate)

1. **spec が二重化しており見出しが 1 つ壊れている (最優先)。** `## 解決済みの論点` から `## 実装分割` までの約 472 行が再掲され、境界の 1 行が `## 上位資料からの変更点` の見出しとスナップショット注記の結合になっていた。結果、未確定事項 / 実装対象 / 機能仕様 / Interface 設計 / Content・Data 設計 / Performance・Security 設計 / Error・Fallback 設計 / テスト評価方針 / フロー・シーケンス / 実装分割が各 2 個あった。**2 つのコピーは同一ではなく**、重複側の「prompts 生成方針」が旧版のままで「P3 以降は直列になる見込み」と書いており、同じ文書内の実装タスク案の並列可と矛盾していた。検証 hook は必須節の存在しか見ないため通ってしまう。前回 PASS 後の編集で入った退行。
2. **`@puppeteer/browsers` が sync で落ちており、変更点表の「反映済」が実態と食い違う。** 変更点表は標準スタック表へ足したと記録しているが、`context/toolchain.md` にはライブラリ名が無かった。spec は closeout で削除されるため、採用パッケージ名と F18 の健全性確認がまとめて失われる状態だった。
3. **P1_01 が採用ライブラリを伏せているため自己完結していない。** 「Chrome for Testing を版指定で取得するツール」とだけ書き、パッケージ名が無い。同 prompt の絶対ルールが探索と推測を禁じているため、実装者はそこで停止せざるを得ない。
4. (勧告) **P5_01 の target 割り当てが spec の責務テーブルから辿れない。** 内容は Store Port と adapter/store だが `domains` に app / store が無く `web` に寄せている。実装対象テーブルの `web` 行に 1 行添えると対応する。

### 確認された整合 (問題なし)

- **prompt の設計仕様と sync 後の上位文書は食い違っていない。** P4_02 は ADR-0028 の決定と、P1_01 は ADR-0027 の改訂内容と一致する。`input-discarded` は web-editor feature と ADR-0008 に定義され、実行イベント列へ混ぜない旨も揃っている。
- **依存関係は実依存として妥当。** 並列可としているのは target と変更ファイルが重ならない組だけである。
- **受け入れ条件 20 件に抜けは検出されなかった。** 各条件が 11 本のいずれかの `## テスト観点` に対応する。
- **sync の反映内容そのものの誤りは指摘 2 の 1 件のみ。** 正本境界は守られ、design 側は規則と ADR への 1 行参照に留まっている。
- 図・決定・変更点表の内容そのものに退行は無く、壊れていたのは文書構造だけである。

### 親 agent の対応

指摘 1 は重複ブロック 469 行を削除し、見出し構造を復元した。削除前に 2 つのコピーを diff して、差分が「prompts 生成方針」だけであることと、残す側が現行版であることを確認した。指摘 2 は `context/toolchain.md` の標準スタック表と ADR-0027 の決定へ `@puppeteer/browsers` を明記し、変更点表の記述を実体に合わせた。指摘 3 は P1_01 の作業ステップと設計仕様にパッケージ名を入れた。指摘 4 は実装対象テーブルの `web` 行に Store Port と adapter/store を含む旨を添えた。

## Review 2026-08-23 (最終 gate 2 回目) — sync + prompts を累積

Verdict: **PASS**

前回の 4 件はすべて解消。**全 8 観点で PASS。**

- 二重化の削除で内容は失われていない。決定は D1〜D28 が 28 件ちょうどで重複も欠落も無く、23 個の見出しは template の並び順どおり。残した側が現行版であることは生成方針の 5 項目で確認できる。
- 他に同種の構造破損は無い。`review.md` も prompts 11 本も必須節が各 1 つ。
- `@puppeteer/browsers` の採用理由は spec 削除後も辿れる。ADR-0027 が採用・版指定取得・`--executable-path` の明示・保守元 / ライセンス / 直接依存数 / deprecated でないこと、および `install` では版を固定できないという却下理由まで持ち、`toolchain.md` は規則と ADR への 1 行参照に留めている。
- 依存関係と並列可の判定は妥当。prompt の設計仕様も ADR-0028 / ADR-0027 / ADR-0008 と一致する。

### 申し送り (closeout / 実装フェーズ向け。判定に影響しない)

1. `session-recreated` は語彙のみで skeleton は発行経路を持たない。実装レビュー時に未実装と取り違えないよう確認する — **P4_01 の作業ステップ・タスク境界・テスト観点の 3 箇所に明記済み**。
2. 記録に使った run の `resume` を承認の前に置く — **User Flow 8 で反映済み** (track gate 4 回目の非ブロッキング推奨で対応)。
3. issue #4 の受け入れ条件との最終突合を closeout 前に 1 度行う — **20 件との突合は親 agent が実施済み** (結果は `index.md` の `## 備考`)。差異は成果物の形式 1 点のみで、issue は形式を指定していないため条件は満たす。
4. 次は closeout。durable 成果は sync 済み、判断は ADR へ反映済みで、spec 削除の前提は満たしている。
