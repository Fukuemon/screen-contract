...
3. 127.0.0.1 のみに bind し、ローカルトークンを要求する。**照合は定数時間で行う** (通常の文字列比較は先頭からの一致長で処理時間が変わり、1 バイトずつ総当たりできる)
4. **接続先ファイルの寿命をプロセスに結びつける。** listen で決まった待受アドレスとポート、生成したトークン、プロセス ID を `runtime.json` へ書き、終了時に削除する。`exit` だけでは足りない (既定のシグナル終了では発火しない) ため `SIGINT` と `SIGTERM` も拾う。読み書きと削除の手段は `P1_01` が `apps/server/src/startup/runtime-file.ts` に用意済みで、`generateLocalToken` も同ディレクトリにある
5. Origin と Host を検査する。`127.0.0.1:<自分の待受ポート>` 以外を拒否する
6. 検証コマンドを実行する
7. diff レビューを回し、指摘を対応する
...
- HTTP API の endpoint と、bind アドレス・トークンの要求・定数時間の照合
- 接続先ファイル (`runtime.json`) の書き出しと、終了時削除のシグナル結線 (`P1_01` から引き継ぐ)
...
- **接続先ファイルは listen の後に書く。** ポートは OS に割り当てさせるため、待受アドレスが決まらないと書けない。`P1_01` は読み書きと削除の手段までを用意し、プロセスの寿命への結線を本 prompt へ送っている。
...
- 接続先ファイル: listen 後に `runtime.json` が 0600 で書かれること。**プロセス終了 (通常終了 / `SIGINT` / `SIGTERM`) で消えること**。これは子プロセスを起動して確かめるため統合テストで行う
