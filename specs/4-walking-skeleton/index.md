...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Why/What → 成功条件         | 「構成番号付き PNG 画像」を「構成番号付きの注釈画像」へ一般化する (source: clarify)                                   | 注釈画像を SVG とし、生スクリーンショットを別ファイルで持つ形に変えるため (D11)                                                                                                                            |
| モジュール責務 → 合成ルート | 「起動と終了の管理」が agent-browser の daemon まで含むと読めないよう、管理範囲をセッションの回収までに限る旨を添える | daemon は CLI プロセスの終了後も生存し、既定は 1 時間のアイドルで終了する (F10)。ADR-0027 は「daemon を明示的に終了させない」と決めており、Design Doc 側だけ読むと合成ルートが daemon を落とすと誤読しうる |
...
| execution → 実行イベント                                                                    | `session-recreated` を語彙へ足す (source: track)                                                                                                                                                                         | D4 が「再作成時にページ状態の喪失をイベントに残す」と定めながら語彙が無い。既存イベントでは再作成して続行したケースを表せない (D25)                                        |
...
| web-editor → Stream の接続構成                                                              | 破棄した入力を表す `input-discarded` を Stream Proxy 側のイベントとして定義する (source: track)                                                                                                                          | ADR-0008 が「破棄した入力をイベントとして残す」と定めながら語彙が無い。run が定まらない破棄があり、実行イベント列へは混ぜられない (D25 / D27)                              |
...
| architecture.md → State Boundary                          | 「draft と確定を分離する」に、**draft と正本を別の置き場に保存する**ことを書き足す。あわせて `(判断の正本は ADR-0017)` の 1 行参照を添える (source: clarify)                                                                                                                                                  | 同節は分離の方針までしか定めていない。`spec-contract.md` は design 側に理由を書かず ADR への 1 行参照を置くと定めるため、規則だけを書いて理由は ADR に残す (D7) |
...
| toolchain.md → 標準スタック / 採用方針                    | `@puppeteer/browsers` と Chrome for Testing の自前取得を標準スタック表へ足す。`agent-browser` の `engines.pnpm >=11` を外れて運用する判断と、**Chrome for Testing の版番号を `packages/adapter-browser` 配下の専用 JSON に置く**ことを記録し、`(判断の正本は ADR-0027)` の 1 行参照を添える (source: clarify) | 同表は「現時点で未確定のものは無い」と宣言しており、依存の追加・engines 逸脱・版の置き場を反映しないと宣言が偽になる (D2 / D21 / D13)                           |
...
| ADR-0008   | 破棄した入力を表す `input-discarded` を Stream Proxy 側のイベントとして定義する旨を追記する (source: track)                                                                                                                                                                                                               | 本 ADR は「破棄したことをイベントとして残す」と定めながら語彙を持たない。実行イベント列は run 単位で決定的であり、run が定まらない破棄を混ぜられない (D25 / D27)                                                                                                      |
| ADR-0017   | 承認依頼の revision を **draft の内容ハッシュ**とする方式を追記する。**draft と正本を別の置き場に保存する判断**と却下案 (同一ファイルの版として持つ形) も 1 項足す (source: clarify)                                                                                                                                      | 本 ADR は「revision を固定する」とだけ定め方式を書いていない。ADR-0018 の IR 版と方式を揃えるため (D9)。置き場の分割は却下案を持つ比較判断で、context へ規則だけを書くと却下案と理由が spec の削除とともに消える (D7)                                                 |
