...
| toolchain.md → 標準スタック / 採用方針                    | `@puppeteer/browsers` と Chrome for Testing の自前取得を標準スタック表へ足す。`agent-browser` の `engines.pnpm >=11` を外れて運用する判断と、**Chrome for Testing の版番号を `packages/adapter-browser` 配下の専用 JSON に置く**ことを記録し、`(判断の正本は ADR-0027)` の 1 行参照を添える (source: clarify) **反映済 (81f4f81)** | 同表は「現時点で未確定のものは無い」と宣言しており、依存の追加・engines 逸脱・版の置き場を反映しないと宣言が偽になる (D2 / D21 / D13)                           |
