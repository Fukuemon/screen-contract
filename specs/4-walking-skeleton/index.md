...
- ADR 起票要否: **要**。注釈画像の形式を SVG とする判断 (D11) を新規 ADR とする。既存 ADR は 0002 (D15 / D19 / D22) / 0008 (D27) / 0017 (D7 / D9) / 0026 (D1 / D5 / D12) / 0027 (D2 / D3 / D13) を改訂する
...
| ADR-0008                         | Stream Proxy の経路と入力転送の検証                                            | 変更提案 (D27 が破棄のイベント語彙を足す)                                                     |
...
| ADR-0028                         | 注釈画像を SVG とし生スクリーンショットを別ファイルで参照する判断              | 新規起票済み (D11 を正本化)                                                                   |
...
**未決の論点は無い。** D1〜D28 はすべて「解決済みの論点」へ移した。新たな論点が出たら本節に表を作り直す。

## 解決済みの論点

> **決定時のスナップショットである。** durable な内容は sync phase で反映先へ書き戻した (`## 上位資料からの変更点` の判定表を参照)。設計の現在値は反映先の文書を、判断の理由は ADR を正本とする。本節は「この issue で何をどう決めたか」の作業記録として残す。
...
| `web`       |    ◯     | 記録の開始と停止、Expectation 候補の選択、承認キュー、live viewport。**Store Port と adapter/store を含む** (`domains` に app / store が無いため `web` に寄せる) |
...
> **正本ハンドオフ済み (81f4f81)。** 下表の全行を反映先へ書き戻した。**以後の正本は反映先の文書**であり、本節と `## 解決済みの論点` は決定時のスナップショットである。設計の現在値を知りたい場合は反映先を読む。判断の理由 (却下案を含む) は ADR に置いた。

### 確定判断の反映先判定

D1〜D28 を全行走査し、durable な反映先を持つものと spec で閉じるものを分けた。sync phase はこの判定に従う。判定は `phase-sync.md` の ADR 化基準 (選択肢を比較して決めた判断は ADR 化を既定とし、spec で閉じられるのは選択の余地がなかった作業上の決定のみ) に照らして付けている。**spec で閉じる**としたものは、いずれも skeleton の範囲判断か既存契約の適用であり、issue が閉じれば判断そのものが意味を失う。**(規則) / (判断)** と 2 段に書いた行は、規則を context へ、却下案と理由を ADR へ分けて置くことを表す。`spec-contract.md` が「design 側は現在の設計だけを持ち、なぜ変えたかは書かない」と定めるためである。

| 判断 | 反映先                                                                                                |
| ---- | ----------------------------------------------------------------------------------------------------- |
| D1   | ADR-0026 / feature doc: element-mapping                                                               |
| D2   | ADR-0027 / context: infrastructure.md / context: toolchain.md                                         |
| D3   | ADR-0027                                                                                              |
| D4   | feature doc: execution                                                                                |
| D5   | ADR-0026                                                                                              |
| D6   | context: testing.md                                                                                   |
| D7   | context: architecture.md (規則) / ADR-0017 (判断)                                                     |
| D8   | **spec で閉じる** (skeleton の実装範囲の判断)                                                         |
| D9   | ADR-0017                                                                                              |
| D10  | context: testing.md                                                                                   |
| D11  | Design Doc: Why/What → 成功条件 / feature doc: artifact-generation / ADR-0028                         |
| D12  | ADR-0026                                                                                              |
| D13  | context: toolchain.md (規則) / ADR-0027 (判断)                                                        |
...
| D25  | feature doc: execution (`session-recreated`) / ADR-0008 + feature doc: web-editor (`input-discarded`) |
| D26  | **spec で閉じる** (D4 の Port 契約は変えず skeleton の方針を固定するだけ)                             |
...
| 対象節              | 変更内容                                                                                                 | 理由                                                                            |
| ------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Why/What → 成功条件 | 「構成番号付き PNG 画像」を「構成番号付きの注釈画像」へ一般化する (source: clarify) **反映済 (81f4f81)** | 注釈画像を SVG とし、生スクリーンショットを別ファイルで持つ形に変えるため (D11) |
...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Why/What → 成功条件         | 「構成番号付き PNG 画像」を「構成番号付きの注釈画像」へ一般化する (source: clarify) **反映済 (81f4f81)**                                   | 注釈画像を SVG とし、生スクリーンショットを別ファイルで持つ形に変えるため (D11)                                                                                                                            |
| モジュール責務 → 合成ルート | 「起動と終了の管理」が agent-browser の daemon まで含むと読めないよう、管理範囲をセッションの回収までに限る旨を添える **反映済 (81f4f81)** | daemon は CLI プロセスの終了後も生存し、既定は 1 時間のアイドルで終了する (F10)。ADR-0027 は「daemon を明示的に終了させない」と決めており、Design Doc 側だけ読むと合成ルートが daemon を落とすと誤読しうる |

### feature doc への影響

| 対象 doc / 節                                                                               | 変更内容                                                                                                                                                                                                                                      | 理由                                                                                                                                                                       |
| ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| execution → Browser Port の契約                                                             | セッションの不応答を表す構造化エラーを Port の語彙に足し、再作成の判断が core にあることを明記する (source: clarify) **反映済 (81f4f81)**                                                                                                     | セッションは部分的に壊れ、生存確認では検出できない (F9)。再現条件も特定できていない (F12)                                                                                  |
| execution → 実行イベント                                                                    | `session-recreated` を語彙へ足す (source: track) **反映済 (81f4f81)**                                                                                                                                                                         | D4 が「再作成時にページ状態の喪失をイベントに残す」と定めながら語彙が無い。既存イベントでは再作成して続行したケースを表せない (D25)                                        |
| execution → 実行状態と再生制御                                                              | run 開始時に pause を予約し完了時に `paused` で止める使い方を明記する。**最終ステップの完了と pause 予約が重なったときは pause を優先する**優先順位と、**pause 予約が 1 回で消費される**ことを書き足す (source: clarify) **反映済 (81f4f81)** | 記録を run の枠内で行い (D15)、再現後の `rerun_step` も `paused` から掛けるため (D22)。優先順位が無いと 1 ステップの run で `completed` に倒れ、記録経路が成立しない (D19) |
| element-mapping → 座標からの要素解決                                                        | 座標解決の入力となる bounding box を `--annotate screenshot` から得ることを明示する。祖先方向の候補列と `label` / `testid` は要素選択の機能であり記録では使わないことを書き分ける (source: clarify) **反映済 (81f4f81)**                      | `snapshot` の応答に box が含まれない (F7)。`--annotate` の応答に祖先関係と `label` / `testid` が無い (D1 / D18)                                                            |
| web-editor → 映像と重ね描きの対応付け                                                       | フレームと Snapshot を結ぶ識別子が agent-browser から得られないことを制約として明記する (source: clarify) **反映済 (81f4f81)**                                                                                                                | 応答にフレーム識別子が無い (F14)。「対応が取れない組み合わせは描画しない」の判定材料を時刻の近似に頼ることになる                                                           |
| web-editor → Stream の接続構成                                                              | 破棄した入力を表す `input-discarded` を Stream Proxy 側のイベントとして定義する (source: track) **反映済 (81f4f81)**                                                                                                                          | ADR-0008 が「破棄した入力をイベントとして残す」と定めながら語彙が無い。run が定まらない破棄があり、実行イベント列へは混ぜられない (D25 / D27)                              |
| workflow-dsl → Schema 検証とバージョン                                                      | 実行系が未対応の語彙を表す構造化エラーを語彙に足す (source: clarify) **反映済 (81f4f81)**                                                                                                                                                     | Schema は語彙を一通り受け付けるが実行系は skeleton の範囲に限るため (D8 / D14)                                                                                             |
| artifact-generation → 背景・要件解釈 / 成果物の種類と生成単位 / 書き換え抑止 / ファイル配置 | 成功条件の再掲「構成番号付き PNG 画像」を一般化し、注釈画像を SVG とする。生スクリーンショットを別ファイルで持って参照する形に変え、書き換え抑止は SVG のテキスト比較へ統一する (source: clarify) **反映済 (81f4f81)**                        | Design Doc の成功条件と同じ文が feature doc にも再掲されており、片方だけ直すとずれる (D11)                                                                                 |

### context への影響

| 対象 doc / 節                                             | 変更内容                                                                                                                                                                                                                                                                            | 理由                                                                                                                                                            |
| --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| architecture.md → Runtime Boundary                        | 「agent-browser は Workflow Server の子プロセスとして adapter/browser が起動・管理する」を実態に合わせる。daemon の生存期間とセッションの回収範囲を書き分ける (source: clarify) **反映済 (81f4f81)**                                                                                | daemon は CLI プロセスの終了後も生存し、既定は 1 時間のアイドルで終了する (F10)。ADR-0027 の「daemon を明示的に終了させない」とも噛み合わない                   |
| architecture.md → State Boundary                          | 「draft と確定を分離する」に、**draft と正本を別の置き場に保存する**ことを書き足す。あわせて `(判断の正本は ADR-0017)` の 1 行参照を添える (source: clarify) **反映済 (81f4f81)**                                                                                                   | 同節は分離の方針までしか定めていない。`spec-contract.md` は design 側に理由を書かず ADR への 1 行参照を置くと定めるため、規則だけを書いて理由は ADR に残す (D7) |
| testing.md → テスト runtime contract / fixture 対象アプリ | agent-browser の隔離は `--namespace` とテスト専用 session 名で行うことを書き足す。**`XDG_STATE_HOME` が agent-browser に効かない**ことを明記する。fixture 対象アプリを静的 HTTP サーバで配信することを書き足す (source: clarify) **反映済 (81f4f81)**                               | 実測で `XDG_STATE_HOME` は無効、`HOME` 差し替えはブラウザキャッシュを失いシステム Chrome へ落ちる (F16)。`file://` は URL に環境依存の値が入る (F15 / D6)       |
| infrastructure.md → 起動時に検査するもの                  | ブラウザ本体の案内を `agent-browser install` から本リポジトリの導入コマンドへ変える (source: clarify) **反映済 (81f4f81)**                                                                                                                                                          | ブラウザ本体を自前で版指定取得するため (D2)                                                                                                                     |
| toolchain.md → 標準スタック / 採用方針                    | `@puppeteer/browsers` (ブラウザ本体の取得) と Chrome for Testing を標準スタック表へ足す。`agent-browser` の `engines.pnpm >=11` を外れて運用する判断と、版番号を `packages/adapter-browser` 配下の専用 JSON に置くことを記録する (source: clarify) **反映済 (81f4f81 / 本 commit)** | 同表は「現時点で未確定のものは無い」と宣言しており、依存の追加・engines 逸脱・版の置き場を反映しないと宣言が偽になる (D2 / D21 / D13)                           |

### ADR の新規 / 更新

| ADR ID   | 変更内容                                                                                                                                                                                                                                                                                                                                       | 理由                                                                                                                                                                                                                                                                  |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ADR-0002 | 決定と状態遷移図に「最終ステップの完了と pause 予約が重なったときは pause を優先する」を追記する。**pause 予約は 1 回で消費される**ことも添える (source: clarify) **反映済 (81f4f81)**                                                                                                                                                         | pause の意味論の正本は本 ADR であり、feature doc だけを直すと状態遷移図が優先順位を持たないまま残って正本が割れる (D19 / D22)                                                                                                                                         |
| ADR-0008 | 破棄した入力を表す `input-discarded` を Stream Proxy 側のイベントとして定義する旨を追記する (source: track) **反映済 (81f4f81)**                                                                                                                                                                                                               | 本 ADR は「破棄したことをイベントとして残す」と定めながら語彙を持たない。実行イベント列は run 単位で決定的であり、run が定まらない破棄を混ぜられない (D25 / D27)                                                                                                      |
| ADR-0017 | 承認依頼の revision を **draft の内容ハッシュ**とする方式を追記する。**draft と正本を別の置き場に保存する判断**と却下案 (同一ファイルの版として持つ形) も 1 項足す (source: clarify) **反映済 (81f4f81)**                                                                                                                                      | 本 ADR は「revision を固定する」とだけ定め方式を書いていない。ADR-0018 の IR 版と方式を揃えるため (D9)。置き場の分割は却下案を持つ比較判断で、context へ規則だけを書くと却下案と理由が spec の削除とともに消える (D7)                                                 |
| ADR-0026 | 未確認事項「入力転送でどこまでの操作種別を扱えるか」を解決済みとして記録する。座標解決に `--annotate` の box を使うこと (D1)、Snapshot を操作の直前に取ること (D5)、Expectation 候補を**操作の前後で変化した項目に絞る**こと (D12) を明記する (source: clarify) **反映済 (81f4f81)**                                                           | `input_mouse` / `input_keyboard` / `input_touch` が使え `click` は実測で通った (F4)。D12 は本 ADR の「操作後の Snapshot から URL・可視要素などの候補を出す」を実質的に狭めるため、書かないと規則が 2 通りになる                                                       |
| ADR-0027 | 未確認事項「Chrome for Testing の版を固定できるか」を解決済みとして記録する。ブラウザ本体を自前で版指定取得し `--executable-path` で明示する方針、起動時検査が案内する導入コマンドの変更、`onlyBuiltDependencies` を足す根拠の書き直しを反映する。**版番号ファイルの置き場の判断**と却下案 3 件も含める (source: clarify) **反映済 (81f4f81)** | `install` では固定できない (F1)。`onlyBuiltDependencies` は「無いと壊れる」ではなく「ネイティブバイナリを直接 spawn し 1 回あたり約 94ms の node 起動を避ける」ことが根拠である (F2 / D3)。版の置き場も比較判断であり、context へ規則だけを書くと理由が残らない (D13) |
| ADR-0028 | 注釈画像を SVG とし、生スクリーンショットを別ファイルで持って参照する判断を新規 ADR として起こした (source: clarify) **反映済 (81f4f81)**                                                                                                                                                                                                      | 選択肢を比較した判断であり、Design Doc の成功条件を変えるため (D11)                                                                                                                                                                                                   |
| ADR-0028 | 生スクリーンショットを**外部参照から `data:` URI の埋め込みへ改める**。実測表 (埋め込み方ごとの読み込み可否) と、二重に持つトレードオフを追記する (source: 実装) **反映済**                                                                                                                                                                    | `<img>` が読み込んだ SVG は外部参照を解決しない。Markdown の `![](x.svg)` は `<img>` になるため、当初の形では画面仕様書に注釈画像を貼れない。GitHub は `<object>` と inline `<svg>` を sanitize で落とすので回避経路も無い                                            |

### 実装で見つかった上位資料の欠落

実装 phase で見つかった、上位資料が要求しているのに DSL / 実装が満たせない項目を記録する。

| 欠落                                                                                                                                 | 正本                                                         | 扱い                                                                                                 |
| ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
| 構成要素テーブルの `optional` 行に載せる `note` (表示条件や特定できない理由)。`ElementDefinition` に該当フィールドが無い             | artifact-generation feature「構成要素テーブルの構成」        | 備考列を空欄にした。DSL への `note` / `unimplemented` の追加は skeleton の範囲外。別 issue へ回す    |
| `all` 指定の要素へ同じ番号のバッジを全件描く規則。DSL に `all` の語彙が無い                                                          | artifact-generation feature「注釈画像の描画規則」            | 未実装。`P4_02` の「実装しない範囲」にも挙がっていない食い違い。別 issue へ回す                      |
| 出力先の 2 段目 (解決後の格納範囲) を実ファイルの symlink で検証すること。core だけでは realpath を呼べない                          | 同 feature「出力先の格納範囲」                               | core は解決済みパスを受ける形にした。symlink の負例は `P7_01` の統合テストで塞ぐ                     |
| 書き換え抑止で **mtime も変わらないこと**。書き込みは Store Port の責務であり core では検証できない                                  | 同 feature「書き換え抑止」                                   | `P5_01` の Store Port 側で塞ぐ                                                                       |
| Stream Proxy (WebSocket) の「接続後の最初のフレームで認証する」経路。`@hono/node-server` の upgrade は Hono のミドルウェアを通らない | [context/infrastructure.md](../../context/infrastructure.md) | 未実装。`P6_01` で Web UI と繋ぐときに、`listen` が upgrade を結線できる形と合わせて入れる           |
| Stream Proxy と記録の結線 (`RunStateSource` の実体、`forwardInput` の呼び出し元)                                                     | web-editor feature                                           | 未実装。`P5_02` は判定と記録の中身までを実装し、経路は `P6_01` が繋ぐ                                |
| HTTP endpoint のうち「記録の開始・停止」「draft の取得」「`rerun_step`」「成果物生成」                                               | `P5_02` の作業ステップ 4-2                                   | 未実装。承認まわりの endpoint のみ入れた。残りは `P6_01` の導線と合わせて足す                        |
| `--annotate screenshot` は `dialog` などのコンテナ自体を返さず、中の要素だけを返す。`element` の Expectation はコンテナを指せない    | 実測 (統合テスト)                                            | 期待状態は「開いたときにだけ現れる子要素」で表す。DSL 側の制約として `context/` へ書き戻す判断は別途 |
