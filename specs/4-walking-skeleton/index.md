...
**未決の論点は無い。** D1〜D18 はすべて「解決済みの論点」へ移した。新たな論点が出たら本節に表を作り直す。
...
- **D15: 記録は、entry まで実行して `paused` にした run の中で行う。**
  ...
  - 根拠 2: **新しい概念が要らない。** execution feature の pause は「実行中ステップの完了後に停止する予約」であるため、run 開始時に pause を予約すれば entry の最後のステップ完了時に `paused` になる。専用の実行モードを作らない。
...
- entry の Workflow 文書は手書きが残る (D17)。「URL を入力すると entry の draft を作る」導線は `#3` の厚くする段階へ送る。**判断者は本 repo の owner、判断時期は skeleton 完了後の次の issue 起票時**とする。
...
2. 利用者が fixture-app を開く entry の Workflow 文書を手で書く (`open` 1 ステップ)。
3. 利用者が run を開始する。pause を予約しておくため、entry の実行完了時に `paused` で止まる。
...
9. 利用者が `run.start` で実行する。記録どおりに操作が再現される。
10. モーダルが開いた状態のまま、同一セッションで最初のステップから `rerun_step` する。全ステップが `skipped` になり `completed` で終わる。
...
- 記録の前提: pause を予約した run が entry の完了時に `paused` で止まること。`paused` でない run への入力転送が破棄され、破棄がイベントに残ること (ADR-0008)。
...
| execution → 実行状態と再生制御                                             | run 開始時に pause を予約し、entry の完了時に `paused` で止める使い方を明記する (source: clarify)                                                                                                   | 記録を run の枠内で行うため (D15)。新しい状態は増やさず既存の pause 予約で表す                                   |
