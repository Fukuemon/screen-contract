...
| 成果物                      | 単位                    | 内容                                                                                                                                    |
| --------------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| 注釈画像 (SVG)              | 画面状態ごとに 1 枚     | 生スクリーンショットを `<image>` で参照し、badges の番号バッジを重ねたもの ([adr/0028](../../../adr/0028-annotated-artifact-format.md)) |
| 生スクリーンショット (PNG)  | 画面状態ごとに 1 枚     | その状態の撮影結果 (clip 指定があれば切り抜き)。**差分検知の対象はこちら**。注釈画像から相対参照される                                  |
...
- **再採番では生スクリーンショットを触らない。** `badges` の並べ替えで変わるのは SVG のテキスト数行だけであり、撮影し直す必要も無い。
...
- **注釈画像は生スクリーンショットを相対参照するため、両者を同じディレクトリに置き、一緒に移動する。** 外部参照する SVG は単体では表示できない ([adr/0028](../../../adr/0028-annotated-artifact-format.md))。
