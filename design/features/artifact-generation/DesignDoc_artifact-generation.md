...
- 本設計が満たすべき成功条件 (DesignDoc の What から):
  - DSL から構成番号付きの注釈画像と Markdown テーブルを生成できる。注釈画像の形式は SVG とし、生スクリーンショットを別ファイルで持って参照する ([adr/0028](../../../adr/0028-annotated-artifact-format.md))。
...
| 成果物                      | 単位                    | 内容                                                                                                                                                         |
| --------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 注釈画像 (SVG)              | 画面状態ごとに 1 枚     | 生スクリーンショットを `<image>` の `data:` URI として埋め込み、badges の番号バッジを重ねたもの ([adr/0028](../../../adr/0028-annotated-artifact-format.md)) |
| 生スクリーンショット (PNG)  | 画面状態ごとに 1 枚     | その状態の撮影結果 (clip 指定があれば切り抜き)。**差分検知の対象はこちら**。注釈画像への埋め込み元でもある                                                   |
...
**撮影と注釈は 2 段に分離する**。撮影 (生スクリーンショット + 各要素の bounding box の取得) は実行時に adapter/browser が行い、注釈 (バッジ配置の計算と SVG の組み立て) は core/artifact の純粋計算である。**core/artifact が生成するのは SVG のテキストであり、画像合成ライブラリを持たない** ([adr/0028](../../../adr/0028-annotated-artifact-format.md))。生スクリーンショットと bounding box は保存し、バッジは何度でも再合成できる — 再採番や badges の並べ替えだけなら再撮影は不要。**差分検知 (Visual Diff) の対象は生スクリーンショットであり、バッジを焼き込んだ注釈済み画像は差分検知の対象にしない** (番号の変更が画面の実変化と混ざるため。詳細は change-detection feature)。
...
- バッジの形状・配色・フォントは固定のスタイル設定 (設定値) とし、要素ごとに変えない。SVG のテキストとして書き出すため、描画の実体は閲覧側のレンダラが担う。
...
- **`index` は 1 始まり、Playwright の `nth()` は 0 始まりである。** 変換時に必ず 1 を引く (`nth(index - 1)`)。引き忘れると、**生成した POM が隣の要素を操作する**。これは実行しても例外にならず、間違った要素に対する操作が静かに成功しうるため、生成時に検出できない。
  - `index` は全ての Locator 戦略で使えるため、**変換は 1 箇所にまとめる**。戦略ごとに書くと片方だけ直し忘れる。
  - `index` が 1 未満、または一致件数を超える場合は**生成エラー**にする。範囲外を `nth()` へ渡すと、実行時まで問題が現れない。
...
生成コードの構造 (workflow-dsl の Screen 例からの生成イメージ):
...
| 成果物                                                   | 比較方法                                                                                                                                     |
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| テーブル / 仕様書 (Markdown) / POM (TS) / 注釈画像 (SVG) | 生成した内容の**テキスト比較 (完全一致)**。すべての成果物が同じ規則で判定される ([adr/0028](../../../adr/0028-annotated-artifact-format.md)) |
| 生スクリーンショット (PNG)                               | 撮影結果 (clip 適用後) のハッシュ比較。撮影し直した結果が同じなら書き換えない                                                                |

- **注釈画像を SVG にしたことで、判定規則が 1 本になった。** 以前は注釈画像だけが「生成入力の比較」という別規則を要していた。PNG は描画エンジンの環境差でバイトが揺れるためである。SVG はテキストなので、Markdown / POM と同じ完全一致で判定できる。
- 「見た目は同じだが入力が変わった」場合 (例: bounding box が 1px 動いた) は書き換えが起きる。閾値による吸収は change-detection の関心であり、生成側では行わない。
- **再採番では撮影し直さない。** `badges` の並べ替えでは保存済みの生スクリーンショットをそのまま使い、SVG を組み直すだけで済む。生スクリーンショットのファイル自体は書き換わらない。
...
└── screens/<screen-id>/
    ...
    ├── <state-id>.svg       # 状態ごとの注釈画像 (raw.png を相対参照する)
    ├── <state-id>.raw.png   # 状態ごとの生スクリーンショット (差分検知の対象)
    ├── <screen-id>.page.ts  # Playwright POM (編集禁止の生成物)
    └── meta.json            # 生成入力の記録 (生スクリーンショットの比較用ハッシュ等)
```

- 配置規則と既定値は core/artifact が決め、書き込みは app が Store Port 経由で行う。
- `meta.json` は書き換え抑止と差分検知のための機械可読情報であり、人間向け文書ではない。
- **注釈画像は生スクリーンショットを埋め込むため単体で表示できる。** それでも両者は同じディレクトリに置く。再採番と差分検知が生スクリーンショットを要るためである ([adr/0028](../../../adr/0028-annotated-artifact-format.md))。
...
- 決定性: 同じ DSL + 同じ撮影結果から、バッジ配置・テーブル内容が常に同一であること。注釈画像 (SVG) がテキストとして完全一致すること。
- バッジ配置: 端・重なり・ずらしの境界ケース。解決不能な重なりの警告。
- テーブル: 番号順、optional 行 (`-` + note) と child_doc リンク行の出力、状態ごとの独立性 (他状態の変更が波及しないこと)。
- 書き換え抑止: 入力同一で書き換えないこと、1 箇所の変更で該当成果物だけが更新候補になること。**再採番で生スクリーンショットが書き換わらないこと**。
- POM の `index` 変換: `index: 1` が `nth(0)` になること。範囲外 (0 以下・一致件数超過) が生成エラーになること。**この 2 件は実行時に例外にならないため、生成側で固定する。**
- 出力先の格納範囲: 絶対パス・`..`・`~` 始まり・解決後にルート外を指すシンボリックリンクが拒否されること。
