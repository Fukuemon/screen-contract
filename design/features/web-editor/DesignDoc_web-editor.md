...
**記録は run の枠内で行う。** モード切替が一時停止中だけ有効である以上、記録を始めるには `paused` の run が要る。画面仕様をゼロから起こす場合は、entry まで実行して `paused` で止めた run の中で記録する。この形なら [adr/0008](../../../adr/0008-stream-proxy.md) の中継条件 (対象 run が `paused` / 操作モード / 要求元のもの) をそのまま満たし、記録した steps をどの状態からの遷移として置くかも run の到達状態から決まる。
...
- **入力転送は server 側でも検証する。** 対象 run が `paused` かつ操作モードであることを Workflow Server が確かめてから中継する ([adr/0008](../../../adr/0008-stream-proxy.md))。client 側の制御だけでは、認証を通したクライアントが Proxy へ直接送って迂回できる。
- **破棄した入力は `input-discarded` として残す。** 破棄の理由 (`paused` でない / 操作モードでない / 要求元の run でない) を含める。**このイベントは実行イベント列 (ExecutionEvent) に混ぜない。** 実行イベントは run 単位で発行順序が決定的だが、破棄は Stream Proxy で起き、「要求元の run でない」場合は結びつけるべき run が定まらない ([adr/0008](../../../adr/0008-stream-proxy.md))。
...
**実行基盤がフレーム識別子を提供するとは限らない。** Snapshot の応答にフレームを特定する値が含まれない場合、対応関係は Workflow Server 側で作る必要がある (撮影の前後にフレームの seq を記録する等)。時刻の近似で代用すると、上の「対応が取れない組み合わせは描画しない」が保証にならない。対応関係を作れない実行基盤では、オーバーレイを出さない方を選ぶ。
...
- 入力の破棄: 中継条件を満たさない入力が破棄され、`input-discarded` に理由が残ること。実行イベント列に混ざらないこと。
