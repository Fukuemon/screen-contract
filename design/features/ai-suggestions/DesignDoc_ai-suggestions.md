...
4. **送信情報の境界** — 既定 Snapshot 断片のみ、設定オプトインで拡張 ([adr/0010](../../../adr/0010-ai-data-boundary.md))。
...
| スクリーンショット画像                                              | 不送信 | プロダクト単位の設定でオプトイン |
| DOM 断片                                                            | 不送信 | プロダクト単位の設定でオプトイン |
...
flowchart TD
    subgraph adapter/ai
        ...
        policy["送信境界フィルタ<br/>(既定 Snapshot・オプトイン適用)"]
...
```mermaid
flowchart TD
    A["core が Port 経由で<br/>候補の補強を要求"] --> B["送信境界フィルタを適用<br/>(オプトイン外の情報を除去)"]
...
- アイコンのみのボタンが多い画面で、利用者がプロダクト設定で画像送信をオプトインし、視覚情報に基づく名称候補を得る。
...
- 送信境界: オプトイン設定ごとに画像・DOM が送信ペイロードから除去されること、secret 系フィールド値が常に除去されること、送信種別が記録されること。
