...
- **`all` 指定の要素は、一致した全件を囲む最小の矩形を代表位置とする。** `all` は複数の bounding box を持つため、どれを使うかを決めないと**同じ入力でも実装によって並びが変わる**。先頭一致を使わないのは、DOM 順が読み順と一致する保証がないためである。
  - 一致件数が 0 のときは幾何情報が無い要素として扱い、末尾へ置いて警告を付ける。
  - バッジは全件へ描くが (成果物生成の規則)、**採番の位置決めに使うのは代表位置 1 つだけ**である。
...
1. 入力: クリック座標 + その時点の Snapshot (Accessibility ツリー + 各ノードの bounding box + 補助 DOM 情報)。取得は adapter/browser の責務。
   ...
   - **観測する要素の範囲。** Accessibility ツリーから、次を落とした残りを観測する。実測 (note.com 4567 ノード) で、**自分も祖先も子孫も観測されない要素は 1 件**だった (読み上げ専用の不可視要素)。

     | 落とすもの                  | 理由                                                 |
     | --------------------------- | ---------------------------------------------------- |
     | `none` / `generic`          | レイアウトの `div`。意味を持たない入れ物             |
     | `InlineTextBox`             | 文字の断片。親の `StaticText` が全体を持つ           |
     | `RootWebArea`               | ページ全体。番号を振る対象ではない                   |
     | 名前が空の要素              | Locator は role と name で引く。名前が無いと指せない |
     | 親と同じ文字の `StaticText` | ボタンのラベル。親として既に数えている               |
     | box が取れない要素          | 表示されていない。枠も番号も置けない                 |
...
**操作の記録も同じ規則を使う** ([adr/0026](../../../adr/0026-operation-recording.md))。記録中の操作は座標として届くため、上と同じ手順で要素へ解決する。違いは 2 点だけである。

- 既存の要素定義に一致すれば、新しい候補を作らずその `ref` を使う。記録のたびに同じ要素が重複定義されないようにする。
- 一意な Locator へ解決できない場合、要素選択ではエラーにするが、記録では `clickPoint` として残して警告を付ける。**記録の途中で止めない**ためである。
