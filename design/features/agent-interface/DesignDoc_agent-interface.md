...
- 認可方式の決定 → Open Question「agent interface の認可方式」(ローカル無認証 / トークン / OAuth)
...
| ------------- | ---------------------------------------------------- | ------------------------------------------- |
| 接続          | エージェント設定に 1 行追加 (標準)                   | 専用クライアント実装が必要 (接続実装は容易) |
| tool 呼び出し | MCP tools (上表と同名)                               | JSON-RPC method (同名・同 Schema)           |
| 実行イベント  | MCP notification で push + `run.events` の取得で補完 | 双方向ストリームでサーバ push (低遅延)      |
| 想定用途      | 汎用エージェントからの操作                           | 常駐・低遅延が要る統合 (エディタ拡張等)     |

- 両プロトコルは同じ tool 語彙・同じ JSON Schema を共有し、agent モジュール内の共通マッピング層が app use case へ変換する。プロトコル固有の処理 (ハンドシェイク、通知の形式) だけを各サーバ実装に置く。
...
- エージェントが `run.start` で再生を開始し、notification で届く ExecutionEvent を見ながら失敗ステップを特定して DSL を修正する。
...
- イベント配信: ExecutionEvent の順序保存、切断・再接続時の取りこぼし補完 (`run.events` での追いつき)。
