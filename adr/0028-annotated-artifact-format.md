...
- **注釈画像は SVG として出力する。** 生スクリーンショットを `<image>` の `data:` URI として**埋め込む** (`href` と `xlink:href` の両方を出し、SVG 2 と SVG 1.1 のどちらのレンダラでも解決させる)。
- **生スクリーンショットは別ファイルとしても保存する。** 差分検知の対象がこちらであり、注釈画像の再合成にも要るためである。
...
本 ADR は当初、生スクリーンショットを相対 path で**外部参照**する形で決定していた。実測 (Chrome for Testing) の結果、その形では成果物が表示できないことが分かったため改訂した。

| 埋め込み方               | 外部参照した `raw.png` の読み込み |
| ------------------------ | --------------------------------- |
| SVG を直接ナビゲートする | 読み込む                          |
| `<img src="x.svg">`      | **読み込まない**                  |
| `<object data="x.svg">`  | 読み込む                          |
| inline `<svg>`           | 読み込む                          |

`<img>` が読み込んだ SVG は外部参照を一切解決しない (secure static mode)。**Markdown の `![](x.svg)` は `<img>` になる**ため、画面仕様書に貼った注釈画像は背景が出ずバッジだけが浮く。GitHub の Markdown は `<object>` と inline `<svg>` を sanitize で落とすので、回避経路も無い。

「`raw.png` と一緒に動かせば表示できる」という当初の前提は誤りであり、同じディレクトリに置いても `<img>` からは表示されない。埋め込みにすると `<img>` / Markdown / 直接参照のいずれでも表示できる。

## 代替案

- **生スクリーンショットを外部参照する**: ファイルが小さく (実測 670 バイト)、再採番の差分も数行に収まる。しかし `<img>` から表示できず、Markdown の画面仕様書に貼れない (上表)。成果物が仕様書に載らないのでは目的を果たさないため却下。
...
- **再採番で撮影し直さない。** `badges` の並べ替えでは保存済みの生スクリーンショットをそのまま使い、SVG を組み直すだけで済む。
- core/artifact が純粋計算のままでいられる。画像合成ライブラリのネイティブ依存が core に入らない。

### 悪い影響 / トレードオフ

- **生スクリーンショットのバイト列が二重に持たれる。** 注釈画像に base64 で埋め込んだものと、差分検知用の `raw.png` である。実測では 16.8KB の PNG に対し SVG が 45.5KB になった (base64 は約 1.33 倍、加えて `href` と `xlink:href` の 2 箇所へ出すため 2 倍)。差分検知の対象は `raw.png` のままなので、増えるのはディスク使用量だけである。
- **再採番でも SVG 全体が書き換わる。** 変わるのはバッジのテキスト数行だが、埋め込んだ画像を含むファイル全体が書き直される。git の差分としては base64 の巨大な行が動かないため読めるが、外部参照のときより差分が大きい。
- Word や Confluence へ貼る運用では PNG が要る。[design/DesignDoc.md](../design/DesignDoc.md) の Goal は配布先を規定していない。PNG が必要になった場合は、ラスタライズを adapter として後から足せる。
...
- spec 更新要否: 要 — `specs/4-walking-skeleton/index.md` の D11 が本 ADR を参照する (実施済み)。埋め込みへの改訂は `## 上位資料からの変更点` に追記する
- context / AI 向け設定更新要否:
