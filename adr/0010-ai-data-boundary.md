# ADR-0010: AI サービスへの送信は既定 Snapshot 断片のみとし設定でオプトイン拡張する
...
- **スクリーンショット画像と DOM 断片の送信は、対象プロダクト単位の設定によるオプトインで許可する** (既定は不許可)。
- パスワード等の secret 系フィールドの値は、オプトイン設定に関わらず送信前に常に除去する。
...
- 対象モジュール / package: element / workflow / infra (adapter/ai の送信境界)

## 実装・運用への反映

- spec 更新要否: 不要 (spec 未作成)
- context / AI 向け設定更新要否:
  - [design/features/ai-suggestions/DesignDoc_ai-suggestions.md](../design/features/ai-suggestions/DesignDoc_ai-suggestions.md) に送信境界の設計を反映 — 本 commit で実施
  - [design/features/element-mapping/DesignDoc_element-mapping.md](../design/features/element-mapping/DesignDoc_element-mapping.md) の AI Port 制約の Open Question 参照を本 ADR 参照へ更新 — 本 commit で実施
  - [design/DesignDoc.md](../design/DesignDoc.md) の Open Question「AI サービスへ送信できる情報」を削除 — 本 commit で実施
