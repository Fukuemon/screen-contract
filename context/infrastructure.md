...
- **既定ではポートを固定しない。** OS に割り当てさせ、結果をこのファイルへ書く。固定すると他のアプリと衝突する。
- プロダクト設定の `port` で固定できる。ブックマークや外部ツールの設定に書きたいときに使う。**指定した番号が使えなければ起動を中止する** — 黙って別のポートへ逃げると、固定した意味が無い。
...
| ファイル形式   | 形式の版 + 鍵の版 + 取り込みの世代 + nonce + 認証タグ + 暗号文               |
| 認証タグ長     | **16 バイトに固定する。** 指定しないと短いタグを受け、偽造の総当たりが下がる |
| 追加認証データ | 形式の版・鍵の版・`authProfile`・世代を AAD に含める                         |
...
取り込みの世代 (generation) は Baseline の識別に入る ([adr/0022](../adr/0022-auth-state-storage.md))。封筒には**平文で持ち、AAD で守る**。平文なのは一覧のたびにキーストアを開かずに済ませるためで、AAD に含めるのは書き換えを検知するためである。

**世代を読むときも復号を通す。** AAD は「その暗号文と組になった世代」を固定するだけであり、復号しない読み手には何も保証しない。読むだけで済ませると、平文の世代を書き換えて Baseline の識別子を任意の値にできる。

### 画面の配信と起動チケット

`GET /` は**トークンを埋め込んだ画面を返す唯一の経路**である。ブラウザは `runtime.json` を読めないため、この経路が要る。

ここを無条件に開けてはいけない。ループバックのポートは**同一マシンの全利用者**が接続でき、`Origin` を送らなければ Origin 検査も素通りする。`curl` 1 本でトークンを取られると、`runtime.json` を 0600 にした意味が消える。

**起動チケットで配る相手を絞る。**

| 項目       | 契約                                                                                    |
| ---------- | --------------------------------------------------------------------------------------- |
| 生成       | 起動ごとに乱数で 32 バイト。**トークンとは別物**で、認可には使わない                    |
| 受け渡し   | 起動時に stderr へ出す URL の `?boot=` に載せる                                         |
| 保持       | 一致したら `HttpOnly` / `SameSite=Strict` の cookie を発行する。以後は query が要らない |
| 一致しない | **トークンを埋め込まない画面を返す。** 画面は開き直しを促す                             |

トークンを query に載せるのとは別である。チケットの寿命はプロセス 1 回分で、これだけでは endpoint を呼べない。「トークンを URL の query に載せない」は変わらない。

### 開発時の画面配信

Web UI は Workflow Server が配信する静的ファイルである。**開発時もこれを崩さない。**

| 契約                                         | dev server から直接開くと    |
| -------------------------------------------- | ---------------------------- |
| Origin と Host は自分の待受ポートだけ許す    | 403 で全滅する               |
| トークンは server が配信する HTML へ埋め込む | 埋め込まれず、画面が動かない |

したがって **Workflow Server が dev server の前に立つ**。画面は server の origin から配られ、中身だけを dev server から取る。API も画面も同じ origin にあり、認可の契約は変わらない。

HMR の WebSocket だけは dev server へ直に繋ぐ。中継すると `/stream` と口が競合し、WebSocket の認可の掛け所が増える。**画面の配信だけを前に立てる。**

dev server の名前空間 (`/@*` / `/src/*` / `/node_modules/*` など) を列挙して中継する。`*` で受けると、後ろに登録する endpoint より先に一致してしまう。列挙から漏れたものは開発時に 404 になるだけで、**認可には影響しない** — トークンを要求する経路はここに入らない。

### 秘密を子プロセスの argv へ載せない

argv は同一利用者の任意プロセスから `ps` で読め、Linux では `/proc/<pid>/cmdline` が他利用者にも見える。暗号化して保管した意味が、渡す 1 ホップで消える。

| 渡すもの         | 経路                                          |
| ---------------- | --------------------------------------------- |
| 暗号鍵           | 標準入力                                      |
| cookie           | 0600 の一時ファイル。使い終わったら消す       |
| Web Storage の値 | **渡さない** (実行基盤に argv 以外の口が無い) |

Web Storage を復元しないことは機能の欠落である。**黙って落とさず**、入らなかったことを利用者へ返す。入ったつもりで未ログインの画面を撮ると、その差分が仕様の変更として記録される。
