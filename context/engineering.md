...
| ツール             | 除外                                                                       | 理由                                                                                                                                                                                                  |
| ------------------ | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| knip               | `packages/adapter-browser` の `agent-browser`                              | 同パッケージは `main` も `exports` も持たず `bin` だけを公開するため、`import` で参照できない。実行時に `require.resolve` でバイナリの実体を指しており、knip はこれを使用として数えない               |
| dependency-cruiser | `apps/web/src/router.tsx` ↔ `apps/web/src/routeTree.gen.ts` の型の循環のみ | 生成物であり手で直せない。TanStack Router は route の型を router へ戻す形で書き出す。**除外は from と via の両方をパスで固定する。** 固定しないと、手書きコードが作った循環でも生成物を経由すれば通る |
