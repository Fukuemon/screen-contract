...
セッションは**テストの間だけ生かす 1 本を共有する**。冪等スキップと再実行は「同じセッションで前提が満たされたままであること」を確かめるものであり、テストごとに作り直すと検証にならない。`run.start` の use case は終了時にセッションを閉じるため、**共有セッションを渡さない**。渡すと後続のテストがセッションを失う。

**`--annotate screenshot` はコンテナ要素自体を返さない。** `<dialog>` を開いても返るのは中の要素だけであり、`element` の Expectation でコンテナを指せない。期待状態は「その状態でだけ現れる子要素」で表す。
...
## agent-browser の実測で分かった性質

公式ドキュメントに書かれていない性質のうち、設計判断が依存するもの。**版を上げたら
確かめ直す** (0.34.0 で実測)。

| 性質                                                                                               | 影響                                                                                                           |
| -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `--annotate screenshot` は box を返す代わりに**対象ページへ赤い枠と番号を描き込む**                | box が要る経路では使えない。`snapshot` で ref を取り `get box` で引く (518 要素 892ms / `--annotate` は 314ms) |
| `snapshot` は描き込まず、`refs` に role と name を返す                                             | 期待状態の候補づくりはこちらを使う                                                                             |
| `storage local set <key> <value>` に argv 以外の口が無い (`@file` も `-` も文字列として保存される) | Web Storage は復元しない。入らなかったことを利用者へ返す                                                       |
| `find role <role> fill --name <name> ""` は値を消して焦点を残す                                    | 秘密を argv へ載せずに入力欄を消せる                                                                           |
| 全選択のキー (`Meta+a`) は効かないことがある                                                       | 消去を全選択に頼らない。効かないと既存の値へ追記され、入力が二重になる                                         |
| 配信への接続は開くまで送れない                                                                     | こちらから送る経路は `open` を待つ                                                                             |
| 同時に 2 つの agent-browser を動かすと、片方の配信が切れることがある                               | **検証で UI を別のブラウザから操作しない。** 映像が止まり、製品の不具合と見分けが付かない                      |
